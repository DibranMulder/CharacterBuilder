...
See [controls and scope](prototypes/training_clearing/README.md).
...
[`docs/characters/human-surface-rig.md`](docs/characters/human-surface-rig.md).
...
2. Lineage-specific body pieces attached to stable pivots/bones.
3. Equipment attached to named sockets (`weapon`, `offhand`, `armor`, `pants`, `boots`, `head`, `back`, `accessory`) with explicit draw layers.
4. Reusable locomotion and weapon-family animations, plus a small set of lineage-specific gesture animations.
...
The authored cloth family—armor, articulated pants, hoods, capes, and scarves—uses a shared cool-blue source palette that is dyed at runtime with each lineage's accent color. This preserves the same painted seams, folds, outlines, and highlights across the modular set while matching the color identities in the two reference sheets. The golden Aeralith tunic/capelet, leather, Sunscour lamellar, plate, wood, metal, and glass retain their authored material colors.
...
Crag Troll receives a deeper overlap plus adapter-owned proximal socket crops
...
Shield equipment now includes three independently swappable painted faces on one boss/grip socket: the general oak-and-brass adventurer shield, Tidekin's reed-lashed marsh buckler, and Sunscour's engraved bronze desert shield. Reference loadouts select the matching construction while every lineage can equip any variant, and climbing carries the selected face onto the back through the same presentation contract.
...
eight slots to prevent gear leaking between lineages, then leaves every supported
...
the Rimeborn reference or the Aeralith's leaf-bearing forked branch staff. Both are
...
The Deep Goblin reference's compact crossbow is also a fully modular two-handed
...
short bodkin bolt is a dedicated authored projectile shared with Deep Goblin's Snap
...
Armor now includes a reference-specific but freely swappable Sunscour lamellar
...
The Aeralith reference's golden wrap tunic, compact red shoulder capelet, blue sash,
...
The Rimeborn's midnight-indigo quilted travel coat is also a freely swappable
...
The Crag Troll now has a dedicated broad charcoal hide jerkin instead of the
...
The Grove Centaur's freely swappable woodland harness replaces generic leather in
...
The Sunscour reference kit pairs its bronze lamellar, burgundy cloth trousers,
...
Tidekin now wears a freely swappable ivory `marsh_tunic` in its reference
...
Aeralith now keeps the painting's approximately Human-height, vertically poised
...
Rimeborn now uses coordinated gray-blue anatomy, white hair, continuous
...
and [surface notes](docs/characters/frostling-surface-rig.md).

Sunscour now uses coordinated reference artwork for its bronze shoulder mantle,
...
and [surface notes](docs/characters/duneborn-surface-rig.md).

Deep Goblin now uses coordinated reference paintings for its large-eared head,
...
[surface and verification notes](docs/characters/troll-surface-rig.md).

Aeralith now uses reference-matched head/ponytail, body and clothing paintings,
...
[centaur rig and motion checks](docs/characters/centaur-surface-rig.md) and
...
See [the human surface rig](docs/characters/human-surface-rig.md) and
