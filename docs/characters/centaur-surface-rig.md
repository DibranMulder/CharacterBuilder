...
bare chest, arms and hands. `CentaurEquineSurface` owns the continuous horse
painting and deformation; `CentaurPaintedAtlas` owns the remaining crops and
magenta key. The head adapter sizes
...
The near horse layer is one indexed 32×36 grid, with shared vertices connecting
the barrel to both legs. CPU skinning blends the upper barrel into the existing
upper/lower leg bone transforms; the knees use a smooth weight transition.
There are no separate near-leg or hoof sprites and no duplicated root edges.
A second, darker mesh samples the same painting's leg areas behind the body,
using the far bones. Its proximal caps blend into the hip and are feathered
under the barrel. This preserves leg crossings without flattening all four
legs into one depth. Indices 0/1 are hind, 2/3 fore; even indices are far-side
and odd indices near-side. The entire rig mirrors for left. Human arms still
use the continuous two-bone strip. All equipment and animation sockets remain.
...
run, climb, jump, rearing and bow. The v4 mesh pass refreshes the side/rear and
reference previews, bare run, left jump and right rearing; other sheets retain
the previous art until regenerated with the same tool. The bare gallop sheet exposes the mesh without
...
`tests/centaur_leg_attachment_regression.gd` checks shared indexed topology,
rest-pose identity, absence of separate leg/hoof rendering, finite deformed
vertices, four skinned hoof endpoints, and climb switching through stand, run,
stairs, jump and rearing in both facings.
