...
func _draw() -> void:
	...
	if front_texture:
		texture = rear_texture if back_view and rear_texture else front_texture
	...
	for row in 12:
		...
		for corner in [Vector2(0, row), Vector2(1, row), Vector2(1, row + 1), Vector2(0, row + 1)]:
			...
			var point := Vector2((uv.x - .5) * size.x * .9, lerpf(-1.1, .14 if centaur else hem_ratio, uv.y) * size.y)
			if centaur:
			...
			uvs.append(uv if centaur or not flip_art else Vector2(1.0 - uv.x, uv.y))
