...
func _run() -> void:
	...
	var appearance: TrollAppearance = avatar.get_node("TrollAppearance")
	assert(BaseAnatomyVisual.HEAD_TEXTURES.frost_troll.resource_path == TrollPaintedAtlas.BASE)
	assert(appearance.jerkin.visible and not avatar._gear.armor.visible)
	assert(avatar._profile.torso.x >= 130 and avatar._profile.limb_width >= 40)
	...
	for facing in [&"right", &"left"]:
		...
		for motion in ModularCharacter.MOTIONS:
			...
			assert(appearance.jerkin.back_view == appearance.body.back_view)
		avatar.play_motion(&"stand")
		for item in CharacterCatalog.items_for(&"armor"):
			avatar.equip(&"armor", item)
			assert(appearance.jerkin.visible == (item == "troll_jerkin"))
			assert(avatar._gear.armor.visible == (item != "troll_jerkin"))
		for item in CharacterCatalog.items_for(&"pants"):
			avatar.equip(&"pants", item)
			var expected := TrollPaintedAtlas.texture("leather_leg" if item == "leather" else "leg")
		...
		for gesture in 3:
			...
			avatar.play_motion(&"stand")
			assert(not appearance.jerkin.back_view)
	avatar.equip(&"weapon", "crossbow")
	assert(avatar.has_node("TrollAppearance"))
	avatar.equip(&"weapon", "axe")
	assert(avatar.has_node("TrollAppearance"))
	avatar.free()
	print("PASS: Troll coordinated surfaces, both facings, all motions, clothing swaps and weapon rebuilds")
	quit()
