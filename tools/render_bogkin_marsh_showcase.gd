...
const OUTPUT_PATH := "res://artifacts/bogkin_marsh_tunic_showcase.png"
const VIEWPORT_SIZE := Vector2i(1200,760)
const TILE_SIZE := Vector2(400,380)
const SAMPLES := [
	{"title":"FRONT / STAND", "mode":&"stand", "time":0.0},
	{"title":"UNIQUE / TONGUE SNAP", "mode":&"gesture", "time":.34},
	{"title":"SWORD / FOREHAND", "mode":&"forehand", "time":.38},
	{"title":"SPRING / RUN", "mode":&"run", "time":.24},
	{"title":"SPRING / STAIRS", "mode":&"stairs", "time":.32},
...
func _add_sample(canvas: Node2D,origin: Vector2,sample: Dictionary,index: int) -> void:
	...
	avatar.scale = Vector2.ONE*1.22
	canvas.add_child(avatar)
	avatar.configure("bogkin",{
		"weapon":"sword", "offhand":"marsh_shield", "armor":"marsh_tunic",
		"pants":"cloth", "boots":"none", "head":"none",
		"back":"none", "accessory":"scarf",
	})
	if sample.mode == &"gesture":
		avatar.play_gesture(0)
	elif sample.mode == &"forehand":
