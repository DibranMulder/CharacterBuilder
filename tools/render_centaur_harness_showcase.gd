...
const OUTPUT_PATH := "res://artifacts/centaur_woodland_harness_showcase.png"
const VIEWPORT_SIZE := Vector2i(1200,760)
const TILE_SIZE := Vector2(400,380)
const SAMPLES := [
	{"title":"FRONT / STAND", "mode":&"stand", "time":0.0},
	{"title":"BOW / FULL DRAW", "mode":&"fire_bow", "time":.42},
	{"title":"BOW / RELEASE", "mode":&"fire_bow", "time":.64},
	{"title":"QUADRUPED / RUN", "mode":&"run", "time":.28},
	{"title":"QUADRUPED / STAIRS", "mode":&"stairs", "time":.36},
...
func _render() -> void:
	...
	if image == null or image.save_png(OUTPUT_PATH) != OK:
		...
		quit(1)
		return
	print("PASS: rendered Grove Centaur woodland harness through bow, quadruped motion, and rear climb to %s" % OUTPUT_PATH)
	quit()
...
func _add_sample(canvas: Node2D,origin: Vector2,sample: Dictionary,index: int) -> void:
	...
	avatar.scale = Vector2.ONE*.82
	canvas.add_child(avatar)
	avatar.configure("centaur",{
		"weapon":"bow", "offhand":"none", "armor":"woodland_harness",
		"pants":"none", "boots":"none", "head":"none",
		"back":"quiver", "accessory":"none",
	})
	if sample.mode == &"fire_bow":
