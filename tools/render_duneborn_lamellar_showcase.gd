...
const OUTPUT_PATH := "res://artifacts/duneborn_lamellar_showcase.png"
...
const SAMPLES := [
	...
	{"title":"REAR / LADDER REACH A", "mode":&"climb", "time":.28},
	{"title":"REAR / LADDER REACH B", "mode":&"climb", "time":.56},
...
func _add_sample(canvas: Node2D,origin: Vector2,sample: Dictionary,index: int) -> void:
	...
	var climbing: bool = sample.mode == &"climb"
	avatar.configure("duneborn",{
		"weapon":"none" if climbing else "spear",
		"offhand":"none" if climbing else "dune_shield",
		"armor":"lamellar", "pants":"cloth", "boots":"leather",
		"head":"none", "back":"none", "accessory":"none",
	})
