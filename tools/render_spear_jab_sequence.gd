...
const OUTPUT_PATH := "res://artifacts/spear_jab_sequence.png"
const VIEWPORT_SIZE := Vector2i(1500,430)
const TILE_SIZE := Vector2(300,430)
const SAMPLES := [
	{"title":"UPRIGHT REST", "time":0.0},
	{"title":"DEEP LOW CHAMBER", "time":.22},
	{"title":"DRIVEN CONTACT", "time":.35},
	{"title":"DECELERATING CARRY", "time":.40},
	{"title":"UPRIGHT RECOVERY", "time":.58},
]
...
func _render() -> void:
	...
	if image == null or image.save_png(OUTPUT_PATH) != OK:
		...
		return
	Engine.time_scale = 1.0
	print("PASS: rendered smoothed Sunscour spear Jab to %s" % OUTPUT_PATH)
	quit()
...
func _add_sample(canvas: Node2D,origin: Vector2,sample: Dictionary,index: int) -> void:
	...
	avatar.position = origin+Vector2(145,370)
